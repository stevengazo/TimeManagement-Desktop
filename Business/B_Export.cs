...
namespace Models
{
	public static class B_Export
	{
		...
		public static async Task<List<Task_Time_Report>> ReportByDayWorkedAsync(DateTime? dateToSearch, int userIdToSearch)
		{
			try
			{
				...
				var queryResults = await (from task in db.TaskItems
										  ...
										  where time.StartTime.Date == dateToSearch && time.EndTime.Date == dateToSearch && user.UserId == userIdToSearch
										  orderby task.UserId
		...
		public static async Task<List<UserTimeReport>> ReportTimeByDayAsync(DateTime? dateToSearch)
		{
			try
			{
				using TimeDatabaseContext db = new TimeDatabaseContext();
				var queryResults = await (from task in db.TaskItems
										  ...
										  select time).Include(time => time.TaskItem)
												...
												.Include(time => time.TaskItem.User)
												.ToListAsync();
				List<UserTimeReport> UserReports = new List<UserTimeReport>();
				var users = (from R in queryResults select R.TaskItem.User).Distinct().ToArray();
				foreach (var item in users)
				{
					UserTimeReport userTimeReport = new() {
					idUser = item.UserId,
					name = item.Name,
					lastname = item.LastName,
					};
					TimeSpan TotalTime = new();
					var taskpeerUser =  (from R in queryResults where R.TaskItem.User.UserId == item.UserId select R).ToList();
                    foreach (var  Times in taskpeerUser)
                    {
						TotalTime = TotalTime + (Times.EndTime - Times.StartTime);                        
                    }
					userTimeReport.date = taskpeerUser[0].EndTime.Date;
					userTimeReport.hours = TotalTime.Hours;
					userTimeReport.minutes = TotalTime.Minutes;
					UserReports.Add(userTimeReport);
                }
				return UserReports;
			}
			catch (Exception ex)
